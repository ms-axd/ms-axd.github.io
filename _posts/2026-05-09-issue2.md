...
Midnight Blizzard 사고의 중요한 점은 고급 공격도 결국 방치된 계정과 예외 환경에서 시작될 수 있다는 것이다.

첫째, 레거시 계정은 시간이 지나면 공격 표면이 된다. 사용하지 않는 테스트 계정이나 오래된 테넌트라도 권한이 남아 있으면 실제 침투 경로가 될 수 있다.

둘째, MFA 예외는 매우 위험하다. 특히 관리자, 테스트, 자동화 계정은 편의상 예외가 생기기 쉬운데 공격자는 이런 계정을 먼저 찾는다.

셋째, OAuth 앱 권한은 비밀번호만큼 중요하게 관리해야 한다. 공격자가 계정을 탈취한 뒤 앱 권한과 토큰을 이용하면 비밀번호를 바꿔도 접근이 계속될 수 있다.

넷째, 이메일은 비밀정보 저장소가 되면 안 된다. 메일에 남은 토큰, 자격증명, 내부 시스템 정보는 2차 공격 재료로 쓰일 수 있다.
