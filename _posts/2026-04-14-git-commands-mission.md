...
```bash
git merge bugFix
# 현재 브랜치에 bugFix 브랜치의 작업을 합친다. 서로 다른 작업 흐름을 하나로 합칠 때 사용.

git rebase main
# 현재 브랜치의 커밋을 main 브랜치 위로 다시 쌓는다. 커밋 기록을 직선 형태로 정리할 때 유용.
```
...
git checkout HEAD~3
#현재 위치에서 세 단계 이전 커밋으로 이동.

git checkout main^
# main 브랜치가 가리키는 커밋의 부모로 이동.
...
git pull
# 원격 변경을 가져오고 현재 브랜치에 합친다. 기본적으로 git fetch 다음 git merge를 실행하는 것과 비슷하다.
...
# origin/main을 기준으로 side 브랜치를 만들고 추적 관계를 설정.

git branch -u origin/main side
# side 브랜치가 origin/main을 추적하도록 설정.

git push origin main
# 로컬 main 브랜치를 원격 origin의 main 브랜치로 올린다.

git push origin main:newBranch
# 로컬 main의 커밋을 원격 newBranch 브랜치로 올린다. 원격 브랜치가 없으면 새로 만들어진다.

git push origin :oldBranch
# 원격의 oldBranch 브랜치를 삭제.
```
...
```bash
git cherry-pick C2
#C2 커밋의 변경만 현재 위치에 복사.

git cherry-pick C2 C4
#여러 커밋을 순서대로 현재 브랜치에 복사.
